...
class GetAllChatMessagesUseCase @Inject constructor(
    ...
    operator fun invoke(
        ...
        try {
            ...
            // version 2
//            Log.d(DebugConstants.TODO, "GetChatMessagesUseCase() - hardcoded id")
//            val messages = version2()


            emit(DataWrapper.Success<List<Message>>(messages))

        } catch(e: HttpException) {
            Log.e("peek", "GetAllChatMessagesUseCase / e1 / ${e.message.toString()}")
            emit(DataWrapper.Error<List<Message>>(e.localizedMessage ?: e.message.toString()))
        } catch(e: java.io.IOException) {
            Log.e("peek", "GetAllChatMessagesUseCase /e2 ${e.message.toString()}")
